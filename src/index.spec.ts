...
test('send BTC', async t => {
    ...
    await new Promise(resolve => txP.on("transactionHash", hash => { console.log(`Got hash!!! ${hash}`); resolve(hash); }))
    await new Promise(resolve => txP.on("confirmation", confirmations => { console.log(`Got confirmation!!! ${confirmations}`); if (confirmations > 0) { resolve(confirmations); } }))
    const tx = await txP;
    await sleep(10 * 1000);
