...
export class ERC20Handler implements Handler<ConstructorOptions, AddressOptions, BalanceOptions, TxOptions> {
    ...
    public readonly handlesAsset = (asset: Asset): boolean => {
        return typeof asset === "object" && asset.hasOwnProperty("type") && (asset as { type: string }).type === "ERC20" && (asset.hasOwnProperty("address") || asset.hasOwnProperty("name"));
    }

    // Address
    public readonly address = async (asset: Asset, options: AddressOptions, deferHandler: DeferHandler): Promise<string> =>
    ...
    public readonly send = (
        ...
        (async () => {
            ...
            const web3PromiEvent = call.send(config) as unknown as PromiEvent<string>;
    ...
    private readonly decimals = async (asset: Asset) => {
        ...
        if (this._decimals[address]) { return this._decimals[address]; }
